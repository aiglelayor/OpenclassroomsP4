...
<section class="container">
    ...
    <table>
      <div class="form-group">
        ...
        if (!isset($_SESSION['pseudo'])) {
          ...
          <small class="form-text text-muted">Vous pouvez également créer un compte, votre pseudo/nom ne vous sera plus
            demandé. Nous serons ravis de vous compter parmi notre communauté de lecteur ! :-)</small>
          <?php
        }
        ?>
      </div>
  ...
  <div>
    ...
    foreach ($comments as $com) {
      ?>
      <p>
        <strong><?= htmlspecialchars($com['author']) ?></strong>
        le <?= htmlspecialchars($com['comment_date_fr']) ?> </br><?= htmlspecialchars($com['comment']) ?>
      </p>
      <?php
      if (isset($_SESSION['pseudo'])) {
        ?>
        <a href="index.php?action=reportComment&comId=<?= $com['id']; ?>&amp;id=<?= $post['id']; ?>">
        ...
        <a href="index.php?action=login">
          <button type="button" class="btn btn-primary">Se connecter pour signaler</button>
        </a>
        <?php
      }
      ?>
