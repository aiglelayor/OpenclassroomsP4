...
<head>
  ...
  <script src="https://cdn.tiny.cloud/1/cjyum6e3ja0scdlceon58esyob6icmcnnw21llwrcjot2pnu/tinymce/5/tinymce.min.js"
...
<div id="newpost_content" class="container white_div my-5 mx-auto p-4 shadow-lg rounded">
  ...
  <form id="newpost_content" class="pb-5 m-auto" action="index.php?action=savePost&id=<?= $_GET['id']; ?>"
        method="post">
    <div class="mb-3">
      <?php
      if (!empty($showErrors)) {
    ...
    <div class="mb-3">
      ...
      <textarea class="form-control" rows="30" name="content" placeholder="Contenu de l'article" id="post_content"
                aria-describedby="" required><?php
        if (isset($post)) {
          echo($post['content']);
        } elseif (isset($_POST)) {
          echo($_POST['content']);
